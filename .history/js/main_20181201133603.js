...
function renderFromData(data){
    if ('content' in document.createElement('template')) {
        ...
        Object.keys(data.compareSummary.titles).forEach((id)=>{
            var clone = document.importNode(headercolumn.content, true);
            img = clone.querySelector('img');
            img.src = data.compareSummary.images[id];
            div = clone.querySelector('div');
            div.textContent = data.compareSummary.titles[id].title;
            spans = clone.querySelectorAll('span');
            spans[0].textContent = data.compareSummary.productPricingSummary[id].finalPrice;
            spans[1].textContent = data.compareSummary.productPricingSummary[id].finalPrice;
            spans[2].textContent = data.compareSummary.productPricingSummary[id].finalPrice;
